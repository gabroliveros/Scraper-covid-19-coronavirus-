...
with open('covid19.csv','w',newline='') as csv_file:
    ...
    writer.writerow(['País','Continente','Contagiados','Nuevos casos','Recuperados',
                     'Fallecidos','Nuevos fallecidos','Activos','Casos críticos',
                     'Casos/millónHab','Muertes/millónHab','Pruebas','Pruebas/mmillónHab',
                     'Población'])
    for fila in tabla.find_all('tr'):
        ...
        for celda in fila.find_all('td'):
            ...
            if nroCelda==3:
                nuevos_casos=remove_punctuation(celda.text)
            if nroCelda==4:
                fallecidos=remove_punctuation(celda.text)
            if nroCelda==5:
                nuevos_fall=remove_punctuation(celda.text)
            if nroCelda==6:
                recuperados=remove_punctuation(celda.text)
            if nroCelda==8:
                activos=remove_punctuation(celda.text)
            if nroCelda==9:
                criticos=remove_punctuation(celda.text)
            if nroCelda==10:
            ...
            if nroCelda==13:
                pruebas_millon=remove_punctuation(celda.text)
            if nroCelda==14:
                poblacion=remove_punctuation(celda.text)
            nroCelda+=1
        writer.writerow([pais,continente,casos,nuevos_casos,recuperados,fallecidos,
                         nuevos_fall,activos,criticos,casos_millon,muertes_millon,
                         pruebas,pruebas_millon,poblacion])
